...
def multiclass_nms(box_delta: Tensor,
                   box_cls: Tensor,
                   test_nms: float,
                   test_cls_threshold: float,
                   num_classes: int,
                   class_aware_box: bool = False,
                   max_output: Optional[int] = None) -> Tuple[List[Tensor], List[Tensor]]:
    
    det_boxes, det_cls, det_label = [], [], []

    for c in range(num_classes):
        inds: Tensor = box_cls[:, c] > test_cls_threshold
        if F.sum(inds) <= 0:
            continue
        scores = box_cls[inds, c]
        bboxes = box_delta[inds, c, :]
        # scores: (n, ) bbox: (n, 4)
        assert 0 < test_nms <= 1.0
        keep = F.vision.nms(bboxes, scores, test_nms, max_output)
        det_cls.append(scores[keep])
        det_boxes.append(bboxes[keep])
        det_label.append(F.full_like(scores[keep], c))

    if len(det_boxes) > 0:
        det_boxes = F.concat(det_boxes)
        det_cls = F.concat(det_cls)
        det_label = F.concat(det_label)
        if det_boxes.shape[0] > max_output:
            det_cls, inds = F.topk(det_cls, k=max_output, descending=True)
            det_boxes = det_boxes[inds]
            det_label = det_label[inds]
    else:
        det_boxes = F.zeros((0, 4))
        det_cls = F.zeros((0, ))
        det_label = F.full_like(det_cls, -1)
    return det_boxes, det_cls, det_label
