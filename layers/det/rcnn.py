...
class RCNN(M.Module):
    ...
    def forward(self, fpn_fms, rcnn_rois, im_info=None, gt_boxes=None):
        # print(f"rcnn_rois.shape: {rcnn_rois.shape}")
        # (1000, 5)
        ...
        pred_logits = self.pred_cls(roi_feature) # (1000, 6)
        pred_offsets = self.pred_delta(roi_feature) # (1000, 20=5 * 4)
        ...
        if self.training:
            ...
            pred_scores = F.softmax(pred_logits, axis=1)[:, 1:] # (1000, 5)
            pred_offsets = pred_offsets.reshape(-1, 4) # (5000, 4)
            target_shape = (rcnn_rois.shape[0], self.cfg.num_classes, 4) # (1000, 5, 4)
            # rois (N, 4) -> (N, 1, 4) -> (N, 80, 4) -> (N * 80, 4)
            base_rois = F.broadcast_to(F.expand_dims(rcnn_rois[:, 1:5], axis=1), target_shape).reshape(-1, 4) # (5000, 4)
            pred_bbox = self.box_coder.decode(base_rois, pred_offsets) # (5000, 4)
            ...
            # ).reshape(-1, self.cfg.num_classes, 4) # (1000, 5, 4)
            # get_det_bboxes = decode -> rescale -> clip -> filter -> multiclass_nms -> (num_rois, 4)
