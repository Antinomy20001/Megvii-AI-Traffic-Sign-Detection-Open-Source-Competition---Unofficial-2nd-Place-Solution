...
from .resnet_mge import ResNetV1d, build_norm_layer, build_conv_layer
from .resnet_mge import Bottleneck as _Bottleneck, ResLayer
...
class RSoftmax(M.Module):
    ...
    def forward(self, x):
        batch = x.shape[0]
        if self.radix > 1:
            x = x.reshape(batch, self.groups, self.radix, -1)
...
class SplitAttentionConv2d(M.Module):
    ...
    @property
    def norm0(self):
        """M.Module: the normalization layer named "norm0" """
    ...
    def forward(self, x):
        ...
        batch = x.shape[0]
        if self.radix > 1:
            splits = x.reshape(batch, self.radix, -1, *x.shape[2:])
            gap = F.sum(splits, axis=1)
        else:
            gap = x
        ...
        if self.radix > 1:
            attens = atten.reshape(batch, self.radix, -1, *atten.shape[2:])
            out = F.sum(attens * splits, axis=1)
        else:
            out = atten * x
        return out
...
class Bottleneck(_Bottleneck):
    ...
    def __init__(self,
        ...
        if self.avg_down_stride:
            self.avd_layer = M.AvgPool2d(3, self.conv2_stride, padding=1)
