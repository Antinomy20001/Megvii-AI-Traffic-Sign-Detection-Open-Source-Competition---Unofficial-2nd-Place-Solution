...
import megengine
import numpy as np
from tools.nms import cpu_batched_nms, rpn_proposals_handle
...
class RPN(M.Module):
    def __init__(self, cfg):
        ...
        rpn_with_dcn = getattr(cfg, 'rpn_with_dcn', False)

        conv_layer = layers.DeformableConv2d if rpn_with_dcn else M.Conv2d

        self.rpn_conv = conv_layer(256, rpn_channel, kernel_size=3, stride=1, padding=1)
    ...
    def find_top_rpn_proposals(
        ...
        for bid in range(im_info.shape[0]):
            ...
            proposals = F.concat(batch_proposal_list, axis=0).detach()
            scores = F.concat(batch_score_list, axis=0).detach()
            levels = F.concat(batch_level_list, axis=0).detach()
            try:
                ...
                bio = BytesIO()
                bio.write(pickle.dumps([proposals, scores, levels]))
                bio.seek(0)
                proposals, scores, levels = pickle.loads(bio.read())
                bio.close()
                del bio
                
                rois = megengine.tensor(rpn_proposals_handle(
                    proposals.numpy(), scores.numpy(), levels.numpy(), self.cfg.rpn_nms_threshold, post_nms_top_n, im_info[bid]
                ))

                print(name, im_info[bid])
