...
import layers
from layers.det import pvt_mge
from functools import partial

class PVTV2(M.Module):

    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        # ----------------------- build backbone ------------------------ #
        bottom_up = getattr(pvt_mge, cfg.backbone)(
...
class PVTV2Config:
    ...
    def __init__(self):
        ...
        self.backbone_pretrained = f'./pretrained_weights/{self.backbone}.pkl'

        self.backbone_freeze_at = 2
        self.fpn_norm = None
        self.fpn_in_features = [0, 1, 2, 3]
        self.fpn_in_strides = [4, 8, 16, 32]
        self.fpn_in_channels = pvt_mge.pvt_config[self.backbone]
