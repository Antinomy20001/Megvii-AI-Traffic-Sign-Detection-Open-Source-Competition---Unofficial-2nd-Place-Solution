...
from tools.nms import class_wise_nms, py_cpu_nms
...
def main():
    ...
    for epoch_num in range(args.start_epoch, args.end_epoch + 1):
        ...
        eval_stats.append([cocoEval.stats[i] for i, m in enumerate(metrics)])
        for cls in range(cfg.num_classes):
            ...
            cocoEval.summarize()
            eval_stats.append([cocoEval.stats[i] for i, m in enumerate(metrics)])
        logger.info("mmAP".center(80, "-"))
        logger.info("|\t%s\t|\t%s\t|"+'\t%s\t|'*cfg.num_classes, 'Metric', 'Total', *[str(i) for i in range(cfg.num_classes)])
        for i, m in enumerate(metrics):
            logger.info('|\t%s\t|' + len(eval_stats) * '\t%.03f\t|', m, *[eval_stats[cls][i] for cls in range(cfg.num_classes + 1)])
        logger.info("|\t%s\t|\t%.03f\t|", 'Score', 0.5 * eval_stats[0][0] + 0.3 * eval_stats[0][2] + 0.2 * eval_stats[0][3])
        logger.info("-" * 80)
...
def worker(
    ...
    for data in test_loader:
        ...
        if tta_transforms is not None:
            ...
            # # remove top arrow
            # ind_c12 = (result[:, -1].astype(int) == 1) | (result[:, -1].astype(int) == 2)
            # keep = ~((result[:, 1] < (origin_height * 0.2)) & ind_c12 & (result[:, 4] > 0.4))
            # # remove bottom sign
            # ind_c034 = ~ind_c12
            # keep &= ~((result[:, 1] > (origin_height * 0.8)) & ind_c034 & (result[:, 4] > 0.4))
