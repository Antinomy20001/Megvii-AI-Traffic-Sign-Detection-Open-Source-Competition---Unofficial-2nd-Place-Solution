...
from megengine.autodiff import GradManager, grad_manager
...
from tools.dataset import BitmapMasks
...
def make_parser():
    ...
    parser.add_argument(
        "-sl", "--sublinear", default=-1, type=int,
    )
...
def worker(args):
    ...
    if args.sublinear != -1:
        ...
        mge.dtr.eviction_threshold = f"{args.sublinear}GB"
        mge.dtr.evictee_minimum_size = 1024 * 1024 * 50
    ...
    start_epoch += 1
    for epoch in range(start_epoch, model.cfg.max_epoch):
...
def train_one_epoch(model, data_queue, opt, gm, epoch, args):
    ...
    def train_func(image, im_info, gt_boxes, gt_masks):
        with gm:
            loss_dict = model(image=image, im_info=im_info, gt_boxes=gt_boxes, gt_masks=gt_masks)
    ...
    for step in range(tot_step):
        ...
        tik = time.time()
        image = mge.tensor(np.ascontiguousarray(mini_batch["data"]))
        im_info = mge.tensor(np.ascontiguousarray(mini_batch["im_info"]))
        gt_boxes = mge.tensor(np.ascontiguousarray(mini_batch["gt_boxes"]))
        gt_masks_ = mini_batch["gt_masks"]
        gt_masks = [BitmapMasks(masks=gt_masks[0], height=gt_masks[1], width=gt_masks[2]) for gt_masks in gt_masks_][0]
        ...
        if dist.get_rank() == 0:
            ...
            if step % log_interval == 0:
                meter_message = meter.average()
                time_message = time_meter.average()
...
def build_dataset(dataset_dir, cfg):
    ...
    data_cfg["order"] = ["image", "boxes", "boxes_category", "info", "mask"]
...
def build_dataloader(batch_size, dataset_dir, cfg):
    ...
    transforms = getattr(cfg, 'train_transforms', None)
    ...
    train_dataloader = DataLoader(
        ...
        collator=MaskDetectionPadCollator(),
        num_workers=4,
    )
