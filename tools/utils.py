...
from tools.nms import class_wise_nms, get_clipped_boxes, filter_boxes
...
class MaskDetectionPadCollator(Collator):
    ...
    def apply(self, inputs):
        """
        assume order = ["image", "boxes", "boxes_category", "mask", "info"]
        """
        batch_data = defaultdict(list)

        for image, boxes, boxes_category, info, mask in inputs:
...
class DetEvaluator:
    ...
    @staticmethod
    def process_inputs(img, short_size, max_size, flip=False, transforms=None):
        ...
        if transforms is not None:
            trans_img = transforms.apply(resized_img)
            trans_img = np.ascontiguousarray(
                trans_img[None, :, :, :], dtype=np.float32
            )
        else:
    ...
    def predict(self, **inputs):
        ...
        """
        box_cls, box_delta, box_label = None, None, None
        pred_output = self.pred_func(**inputs)
        if len(pred_output) == 2:
            box_cls, box_delta = pred_output
        elif len(pred_output) == 3:
            box_cls, box_delta, box_label = pred_output
            # try:
            #     box_label = box_label.numpy()
            # except:
            #     print(f"box_cls:{box_cls.shape}, box_delta:{box_delta.shape}, box_label:{box_label.shape},")
        # box_cls, box_delta = self.model(**inputs)
        dtboxes_all = list()
        nms_method = getattr(self.model.cfg, 'final_nms_method', 'multiclass')

        assert nms_method in ['multiclass', 'normal'], 'final_nms_method must be "multiclass" or "normal"'
        
        if box_cls.shape[0] > 0:
            box_cls, box_delta = box_cls.numpy(), box_delta.numpy()

        else:
            return np.empty((0, 6))

        if nms_method == 'normal':
            # (x1, y1, x2, y2, score, category)
            box_label = box_label.numpy()
            im_info = inputs.get('im_info')
            box_delta = get_clipped_boxes(box_delta, im_info[0, 2:])
            keep = filter_boxes(box_delta)
            box_delta = box_delta[keep]
            box_cls = box_cls[keep]
            box_label = box_label[keep]
            dtboxes = np.hstack((box_delta, box_cls.reshape(-1, 1), box_label.reshape(-1, 1))).astype(np.float32)
            # keep = class_wise_nms(dtboxes, self.model.cfg.test_nms, self.model.cfg.num_classes)
            # dtboxes = dtboxes[keep]
            
            dtboxes_all.extend(dtboxes)
        else:
            # (1000, 5) (1000, 5, 4), (num_rois, num_classes, 4)
            ...
            for c in range(self.model.cfg.num_classes):
                ...
                if dtboxes.size > 0:
                    ...
                    # dtboxes: (k, 6), (x1, y1, x2, y2, score, category)
                    dtboxes_all.extend(dtboxes)
        ...
        dtboxes_all = np.array(dtboxes_all, dtype=np.float) # (k, 6)
        return dtboxes_all
    ...
    def format(results, cfg):
        ...
        all_results = []
        import tqdm
        for record in tqdm.tqdm(results):
